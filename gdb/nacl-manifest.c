...
static struct file_list *nacl_file_list = NULL;


static char *nacl_program_filename = NULL;
...
nacl_manifest_find (const char *original_name)
{
  ...
  if (strcmp(original_name, "NaClMain") == 0 /* ||
      strcmp(original_name, "ld-nacl-x86-32.so.1") == 0 ||
      strcmp(original_name, "ld-nacl-x86-64.so.1") == 0 */)
    {
      if (nacl_program_filename)
        return nacl_program_filename;
    }
